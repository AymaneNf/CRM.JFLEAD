...
public class Program
{
    public static void Main(string[] args)
    {
        ...
        app.MapPut("/api/leads/{id}", async (int id, [FromBody] Lead lead, ILeadService leadService) =>
        {
            if (id != lead.Id)
            {
        ...
        app.MapDelete("/api/leads/{id}", async (int id, ILeadService leadService) =>
        {
            var deletedLead = await leadService.DeleteLeadAsync(id);
            return deletedLead != null ? Results.Ok(deletedLead) : Results.NotFound();
        });

        app.MapGet("/api/leads/{id}", async (int id, ILeadService leadService) =>
        {
        ...
        app.MapPost("/api/leads/{id}/assign", async (int id, [FromBody] int collaboratorId, ILeadService leadService) =>
        {
            await leadService.AssignLeadAsync(id, collaboratorId);
            return Results.NoContent();
        });

        app.MapPost("/api/leads/{id}/start", async (int id, ILeadService leadService) =>
        {
            await leadService.StartLeadAsync(id);
            return Results.NoContent();
        });

        app.MapPost("/api/leads/{id}/convert/won", async (int id, ILeadService leadService) =>
        {
            await leadService.ConvertLeadToWonAsync(id);
            return Results.NoContent();
        });

        app.MapPost("/api/leads/{id}/convert/lost", async (int id, ILeadService leadService) =>
        {
            await leadService.MarkLeadAsLostAsync(id);
            return Results.NoContent();
        });

        app.MapPost("/api/leads/{id}/event", async (int id, [FromBody] string eventDetails, ILeadService leadService) =>
        {
